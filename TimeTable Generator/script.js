...
function addTimetableEntry(subject, startTime, endTime, selectedDays) {
    ...
    if (rowExists) {
        selectedDays.forEach(day => {
            const dayIndex = getDaysIndex(day);
            targetRow.cells[daysIndex].innerHTML = subject;
        });

    } else {
        const newRow = document.createElement('tr');
        newRow.innerHTML = '<td>${startTime}-${endTime}</td><td></td><td></td><td></td><td></td><td></td>';
        selectedDays.forEach(day => {
...
function insertSortedRow(newRow, startTime, timetableBody) {
    ...
    if (existingRows.lengthlenght === 0) {
